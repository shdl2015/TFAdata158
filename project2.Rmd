...
The scatter plot of Difference in Math Test Scores vs. Total Hours of Teacher Professional Development implies that we cannot fit a linear regression model on these two variables, because the explanatory variable is not continuous. In addition, each teacher, no matter how many hours of professional development, will have their own distribution of student math scores. This explains why there were mutiple test scores for the same amount of teacher training.  

##### Confidence Interval
```{r}
newtest <- data.frame(atotal=0)
...
There are multiple test scores observed when a teacher has had zero hours of professional development, so we were unable to construct a confidence interval. 
...
Due to the nature of our variables, a linear regression does not fit. 